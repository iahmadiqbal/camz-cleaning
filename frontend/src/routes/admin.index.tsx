          ...
          {stats.map((s, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="w-10 h-10 rounded-lg bg-primary text-white grid place-items-center"><s.icon className="w-5 h-5" /></div>
            ...
            <div className="space-y-3">
              {bookings.slice(0, 5).map((b) => (
                <div key={b.id} className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-xs px-2 py-1 rounded-full bg-primary text-white font-medium">{b.status}</span>
                </div>
              ))}
            </div>
          </motion.div>
