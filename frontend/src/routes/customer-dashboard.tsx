...
// Demo: use first customer's bookings
        ...
        <div className="max-w-5xl mx-auto px-6 py-12">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-sm">Welcome back, Sarah</p>
            </div>
            <Link to="/services" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[image:var(--gradient-hero)] text-primary-foreground font-medium shadow-[var(--shadow-card)] text-sm">
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {[
              ...
              <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-card)] text-center">
                <div className="w-10 h-10 rounded-lg bg-soft-blue text-primary grid place-items-center mx-auto mb-2"><s.icon className="w-5 h-5" /></div>
            ...
            {active.length === 0 ? (
              ...
              <div className="space-y-3">
                {active.map((b, i) => {
                  ...
                  return (
                    <motion.div key={b.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                      className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-card)] flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <div className="w-11 h-11 rounded-xl bg-soft-blue text-primary grid place-items-center flex-shrink-0"><Icon className="w-5 h-5" /></div>
                        <div>
                          <div className="font-semibold text-deep-blue">{b.service} Cleaning</div>
                          <div className="text-xs text-muted-foreground mt-0.5">{b.date} · {b.staff !== "—" ? `Cleaner: ${b.staff}` : "Awaiting assignment"}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 flex-shrink-0">
                        <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusColor(b.status)}`}>{b.status}</span>
                        <Link to="/tracking" className="text-xs text-primary hover:underline flex items-center gap-1"><MapPin className="w-3 h-3" />Track</Link>
                      </div>
                    </motion.div>
                  );
                })}
              ...
              <div className="space-y-3">
                {past.map((b, i) => (
                  <motion.div key={b.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                    className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-card)] flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className="w-11 h-11 rounded-xl bg-green-100 text-green-700 grid place-items-center flex-shrink-0"><CheckCircle2 className="w-5 h-5" /></div>
                      <div>
                        <div className="font-semibold text-deep-blue">{b.service} Cleaning</div>
                        <div className="text-xs text-muted-foreground mt-0.5">{b.date} · ${b.amount} · {b.staff}</div>
                      </div>
                    </div>
                    <Link to={`/booking/$service` as never} params={{ service: b.service.toLowerCase().replace(" ", "") } as never}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border text-xs font-medium hover:bg-soft-blue transition-colors">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-8">
            ...
            <div className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)] space-y-4">
              <div><label className="block text-sm font-medium mb-1.5">Special instructions</label><textarea className="w-full px-4 py-2.5 rounded-lg border border-border bg-background text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/40" rows={2} placeholder="e.g. Please use fragrance-free products" /></div>
              <div><label className="block text-sm font-medium mb-1.5">Entry details</label><input className="w-full px-4 py-2.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/40" placeholder="e.g. Key under mat, door code 1234" /></div>
              <div><label className="block text-sm font-medium mb-1.5">Pet information</label><input className="w-full px-4 py-2.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/40" placeholder="e.g. 1 dog, friendly, keep door closed" /></div>
